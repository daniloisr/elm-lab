...
Elm.Tabs = Elm.Tabs || {};
Elm.Tabs.make = function (_elm) {
   "use strict";
   _elm.Tabs = _elm.Tabs || {};
   if (_elm.Tabs.values) return _elm.Tabs.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var getTabs = Elm.Native.Port.make(_elm).inbound("getTabs",
   "List Tabs.Tab",
   function (v) {
      return typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "title" in v && "url" in v && "id" in v && "active" in v ? {_: {}
                                                                                                    ...
                                                                                                    v.active)} : _U.badPort("an object with fields `title`, `url`, `id`, `active`",
         v);
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "ChangeTab": var updateTab = function (t) {    return _U.eq(t.id,_p0._0) ? _U.update(t,{active: true}) : _U.update(t,{active: false});};
           return _U.update(model,{tabs: A2($List.map,updateTab,model.tabs)});
         default: return _U.update(model,{lastKey: _p0._0});}
   });
   var initModel = function (tabs) {    return {tabs: tabs,lastKey: 0};};
   var initialModel = initModel(getTabs);
   var Tab = F4(function (a,b,c,d) {    return {title: a,url: b,id: c,active: d};});
   var Model = F2(function (a,b) {    return {tabs: a,lastKey: b};});
   var KeyPressed = function (a) {    return {ctor: "KeyPressed",_0: a};};
   var ChangeTab = function (a) {    return {ctor: "ChangeTab",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var model = A3($Signal.foldp,
   update,
   initialModel,
   A2($Signal.merge,A2($Signal.map,$Basics.identity,actions.signal),A2($Signal.map,KeyPressed,$Keyboard.presses)));
   var changeTab = Elm.Native.Port.make(_elm).outboundSignal("changeTab",
   function (v) {
      return {tabs: Elm.Native.List.make(_elm).toArray(v.tabs).map(function (v) {    return {title: v.title,url: v.url,id: v.id,active: v.active};})
             ,lastKey: v.lastKey};
   },
   model);
   var view = function (model) {
      ...
      _U.list([A2($Html.ul,
              ...
              A2($List.map,
              function (x) {
                 return A2($Html.li,_U.list([A2($Html$Events.onClick,actions.address,ChangeTab(x.id))]),_U.list([$Html.text(x.title)]));
              },
              model.tabs))
              ,$Html.text($Basics.toString(model.lastKey))]));
   };
   var main = A2($Signal.map,view,model);
   var keyPressed = A2($Signal.map,function (x) {    return A2($Signal.message,actions.address,KeyPressed(x));},$Keyboard.presses);
   return _elm.Tabs.values = {_op: _op
                             ,NoOp: NoOp
                             ,ChangeTab: ChangeTab
